...
public class LITPlogrotateTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPlogrotateTestRunner.class);
    ...
    @TestId(id = "CXP9031121-1", title = "Upgrade ERIClitplogrotate rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitplogrotateRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("logrotate");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031121-2", title = "Run python test cases for ERIClitplogrotate package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitplogrotateTests() {
     ...
     * @PRE Execution of test {@link #runERIClitplogrotateTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031121-3", title = "Parse xml outputs from python tests")
